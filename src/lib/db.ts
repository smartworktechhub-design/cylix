...
export async function getUserPackages(userId: string): Promise<UserPackage[]> {
  const { data } = await supabase.from('packages').select('*').eq('user_id', userId).order('activated_at', { ascending: false });
...
export async function getUserEarnings(userId: string): Promise<Earnings> {
  const { data } = await supabase.from('earnings').select('type, amount').eq('user_id', userId);
...
export async function getReferrals(userId: string): Promise<Referral[]> {
  const { data } = await supabase.from('users').select('id, wallet, created_at, total_earned, team_size').eq('referred_by', userId);
...
export async function getLeaderboard(limit = 10): Promise<any[]> {
  const { data } = await supabase.from('users').select('wallet, total_earned, team_size').order('total_earned', { ascending: false }).limit(limit);
...
export async function getAdminStats(): Promise<any> {
  ...
  const { data: pkgData } = await supabase.from('packages').select('invested').eq('status', 'active');
  const { data: wdData } = await supabase.from('withdrawals').select('amount, status');
